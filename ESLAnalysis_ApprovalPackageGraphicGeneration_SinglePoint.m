...
ELS_dataFolder =strcat(ELSFolder, '\Data_20190531\', DPAfolderName);
...
escnum = '\nPlease enter the ESC number: ';
SelectedESCsensorCombination = input(escnum);
if isempty(SelectedESCsensorCombination)
    SelectedESCsensorCombination = 1;
end
...
CoverageReliabilityThreshold50 = 0.5;            % Reliability threshold of 0.50 for DPA Coverage percentage calculation
...
for DPA_gridIndex = 1 : gridPointCount          % Color each of the Grid Points according to the achieved Reliability
    ...
    gridPoint_Reliability = GridPointDetectionInfo{SelectedESCsensorCombination}(DPA_gridIndex,2);
    ...
    if ge(gridPoint_Reliability, 0.5) && lt(gridPoint_Reliability, 0.95)
